...
<template>
    <div id="app">
        ...
        <div class="bankTransferList">
            <bank-transfer :banks="banks" @add:bank="addBanks" @calculate:selected="calculateSelected()" />
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        calculateSelected() {
            const selectedBudgets = this.budgets.filter(budget => budget.selected === true)
            const totalSelectedBudgets = selectedBudgets.reduce((total, budget) => {
                return total + budget.amount
            }, 0)
            const totalIncomes = this.incomes.reduce((total, income) => {
                return total + income.amount
            }, 0)
            const totalBanks = this.banks.reduce((total, bank) => {
                return total + bank.amount
            }, 0)
            const total = totalIncomes - totalSelectedBudgets - totalBanks
            const totalPerBank = total / this.banks.length
            const newBanks = this.banks.map(bank => {
                return {
                    ...bank,
                    amount: totalPerBank
                }
