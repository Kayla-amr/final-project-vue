                    ...
                    <tr>
                        <th>
                            <div class="bankName py-2">
                                <input class="bg-base-200 p-2" type="text" id="bankName" v-model="bank.name"
        ...
        <table class="table">
            <thead>
                <tr>
                    <th>Bank Name</th>
                    <th>Amount</th>
                    <th>Add <br>Expenses</th>
                    <th>Delete <br> Bank</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="bank in banks" :key="bank.id">
                    <td>{{ bank.name }}</td>
                    <td>${{ bank.amount }}</td>
                        <td class="button"><button name="arrow-forward-outline" class="btn btn-primary" type="transfer"
                                @click="transferSelectedBudget(bank.id)">+</button></td>
                        <td class="button"> <button class="btn btn-error" type="delete"
                                @click="removeBank(bank.id)">x</button></td>
                </tr>
            </tbody>
            <tfoot>
                <tr>
                    ...
                    <td><button class="btn btn-error" type="clear" @click="clearTransfer">Clear <br>Amount</button></td>
                </tr>
            </tfoot>
        </table>
    </div>
