...
export default {
  ...
  transferSelectedBudget() {
    ...
    const totalSelectedBudgetAmount = selectedBudgets.reduce((total, budget) => total + budget.amount, 0)
    const bank = this.banks.find(bank => bank.name === this.bank.name)
    bank.amount += totalSelectedBudgetAmount
