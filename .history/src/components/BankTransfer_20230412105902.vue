...
export default {
    ...
    const selectedBudgets = this.budgets.filter(budget => budget.selected && !budget.addedToBank); // filter only selected budgets that have not been added to the bank
    const totalSelectedBudgetsAmount = selectedBudgets.reduce((total, budget) => {
        return Math.ceil(total + budget.amount / 2);
    }, 0);

    if (selectedBudgets.length > 0) {
        const bank = this.banks.find(bank => bank.name === this.bank.name); // find the bank with the selected name
        if (bank) {
            bank.amount += totalSelectedBudgetsAmount;
            selectedBudgets.forEach(budget => {
                budget.addedToBank = true; // set addedToBank flag to true for selected budgets
            });
        }
    }
}
