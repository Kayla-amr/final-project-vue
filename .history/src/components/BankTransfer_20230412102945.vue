    ...
    methods: {
        ...
        transferSelectedBudget() {
            ...
            if (selectedBudgets.length > 0) {
                if (totalSelectedBudgetsAmount > this.total) {
                    alert("You don't have enough money in your bank accounts to transfer the selected budgets");
                } else {
                    const bank = this.banks.find(bank => bank.id === 1);
                bank.amount += totalSelectedBudgetsAmount;

                this.banks = this.banks.filter(bank => bank.id !== 1);
