...
<template>
    ...
    <div id="app">
        ...
        <div class="incomeSplitList">
            <split-budget :budgets="budgets" @add:income="addIncomes" :incomes="incomes" @split:budget="SVGComponentTransferFunctionElement"/>
        </div>
...
export default {
    ...
    methods: {
        ...
        splitAmount() {
            const split = this.budgets.filter(budget => budget.amount > 0)
            const total = split.reduce((total, budget) => {
                return total + budget.amount
            }, 0)
            const totalDivided = total / 2
            const splitAmount = split.map(budget => {
                return {
                    ...budget,
                    amount: Math.ceil(budget.amount / totalDivided)
                }
            })
            this.splitBudgets = splitAmount
